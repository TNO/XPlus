 ...
 */
package nl.esi.xplus.xtext.generator.serializer
...
class PatchedSerializerFragment2 extends SerializerFragment2 {

    static val Logger LOG = LoggerFactory.getLogger(PatchedSerializerFragment2)
...
// FIXME: Start re-expose public setters
    override setGenerateStub(boolean generateStub) {
        super.setGenerateStub(generateStub)
    }
// END: Start re-expose public setters
...
// FIXME: Start of patch for Xtext issue 1991
        return if (isMapEntry(type)) {
            '''
				sequence_«constraint.simpleName»(context, («type» & EObject) semanticObject); 
				return; 
			'''
        } else {
        '''
...
// FIXME: End of patch for Xtext issue 1991
    }
	...
	//
	««« FIXME: Start of patch for Xtext issue 1991 
        // «genMethodSequenceDeclaration(c)» { }
	««« FIXME: End of patch for Xtext issue 1991
    '''
...
// FIXME: Start of patch for Xtext issue 1991
    private def StringConcatenationClient genMethodSequenceDeclaration(IConstraint c) {
        return if (isMapEntry(c.type)) {
            '''protected <T extends «c.type» & EObject> void sequence_«c.simpleName»(«ISerializationContext» context, T semanticObject)'''
        } else {
            '''protected void sequence_«c.simpleName»(«ISerializationContext» context, «c.type» semanticObject)'''
        }
    }

    private def boolean isMapEntry(EClass type) {
        return type.instanceClassName == Map.Entry.name
    }
// FIXME: End of patch for Xtext issue 1991
